...
from rag_lab.evaluation.rerank_cli import main
...
class FakeEmbeddingProvider:
    ...
    def embed_documents(
        ...
        raise AssertionError(
            "evaluate-rerank must not embed documents"
        )
...
class FakeVectorStore:
    ...
    def ensure_collection(self) -> None:
        raise AssertionError(
            "evaluate-rerank must not create collections"
        )
    ...
    def upsert(
        ...
        del records
        raise AssertionError(
            "evaluate-rerank must not index records"
        )
    ...
    def search(
        ...
        *,
        ...
        return SearchResult(
            ...
            retriever="hybrid+rerank",
            index_version=(
                "hybrid-v1:index|rerank-v1:lexical-overlap"
            ),
        )
...
def test_writes_human_report_and_routes_configuration(
    ...
    reranked_config: list[dict[str, object]] = []
    ...
    def reranked_retriever_factory(
        **kwargs: object,
    ) -> FakeRetriever:
        reranked_config.append(kwargs)
        return retriever
    ...
    assert len(reranked_config) == 1
    assert isinstance(
        reranked_config[0]["retriever"],
        HybridRetriever,
    )
    assert isinstance(
        reranked_config[0]["reranker"],
        LexicalOverlapReranker,
    )
    assert reranked_config[0]["fetch_k"] == 20
...
def test_writes_json_report(
    ...
    exit_code = main(
        ...
        reranked_retriever_factory=lambda **_: retriever,
    )
...
def test_reports_factory_error(
    ...
    exit_code = main(
        ...
        store_factory=lambda **_: FakeVectorStore(),
        reranked_retriever_factory=lambda **_: FakeRetriever(
            chunks_by_query={}
        ),
    )
...
def test_missing_collection_returns_exit_two_without_creation(
    ...
    collection_name = "missing-evaluate-rerank"
