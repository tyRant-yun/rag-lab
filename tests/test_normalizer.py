...
def test_repeated_same_chapter_marker_keeps_heading_path(
    ...
    document = {
        ...
        "texts": [
            ...
            text_item(3, text="第一页正文。", page=1, top=540),
            text_item(
                4,
                text="第 1 章",
                page=2,
            ...
            text_item(7, text="第二页正文。", page=2, top=540),
        ],
    }
    ...
    second_section = next(
        block for block in result.blocks if block.text == "1.2 第二节"
    )
    false_heading = next(
        block for block in result.blocks if block.text == "历史事件"
    )
    ...
    assert false_heading.block_type == BlockType.PARAGRAPH.value
